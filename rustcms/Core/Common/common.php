...
function D($name='',$layer='') {
    ...
    static $_model = array();
    $layer=!empty($layer)?$layer:C("DEFAULT_M_LAYER");
    ...
    if((count($path)>3)&&(1==C("APP_GROUP_MODE"))){
    	$baseUrl=($path[0]=='@')?BASE_LIB_PATH:APP_PATH.'../'.$path[0].'/'.C("APP_GROUP_PATH").'/';   	 
...
function A($name,$layer='',$common=false) {
    ...
    $layer=!empty($layer)?$layer:C("DEFAULT_C_LAYER");
   ...
   $path=explode('/', $name);
    ...
    import(str_repeat('@/','',$name).$layer,LIB_PATH);
    }else{
    import($name.$layer);
    }
    $class   =   basename($name.$layer);
